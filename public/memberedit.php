...
if (count($_POST)) {
    ...
    $firstname = filter_input(INPUT_POST, 'firstname', FILTER_SANITIZE_SPECIAL_CHARS); // Haal 'firstname' op en verwijder speciale tekens.
    $lastname = filter_input(INPUT_POST, 'lastname', FILTER_SANITIZE_SPECIAL_CHARS); // Haal 'lastname' op en verwijder speciale tekens.
    if (!$firstname) {
    ...
    $payed = filter_input(INPUT_POST, 'payed'); // Haal 'payed' op

    $email = filter_input(INPUT_POST, 'email'); // Haal 'email' op.
    ...
    $telephonenumber = filter_input(INPUT_POST, 'telephonenumber'); // Haal 'telephonenumber' op
    ...
    if (count($errors) == 0) {
        ...
        if ($id) {
            ...
            updateMember($id, $payed, $firstname, $lastname, $email, $streetname, $streetnumber, $town, $zip, $telephonenumber);
        } else {
        ...
        header('Location: members.php'); // Stuur de gebruiker door naar de ledenlijst.
        exit;
    }
}
?>
...
<section class="wrapper">
    ...
    <div class="header-breadcrumbs">
        <div>
            ...
            <span>/</span>
            <a class="active-page" href="#">Lid Bewerken</a>
        </div>
    ...
    <h1 class="header-titles top">Lid Aanpassen</h1>
        ...
        <div class="member-flex">
            ...
            <div class="member-flex--item">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="email" id="email" class="form-control" value="<?= $_POST['email'] ?? $member->email ?? '' ?>">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="town" id="town" class="form-control" value="<?= $_POST['town'] ?? $member->town ?? '' ?>">
                </div>

                <div class="form-group form-group--small">
                    <label for="zip">Postcode</label>
                    <input type="text" name="zip" id="zip" class="form-control" value="<?= $_POST['zip'] ?? $member->zip ?? '' ?>">
                </div>
